...
interface AssignmentRequestsPanelProps {
  // Where "Place on timetable" (on an ALLOCATED incoming request) sends this
  // viewer - the HOD's own grid at /hod/timetable/... or a Timetable
  // Incharge's own equivalent under /panel or /college-staff (see
  // TimetableIncharge in src/types/core.ts). Kept as a prop rather than
  // hardcoded so this one component serves all three routes.
  timetableHrefFor: (r: FacultyAssignmentRequest) => string;
}

// Shared by hod/assignment-requests, panel/assignment-requests and
// college-staff/assignment-requests - an HOD and any Timetable Incharge for
// a department (any course-year, see isTimetableInchargeForDepartment) both
// fulfill/decline incoming requests and track their own outgoing ones the
// same way. The API (faculty-assignment-requests/route.ts GET,
// [id]/route.ts PATCH) already scopes what each caller can see/act on -
