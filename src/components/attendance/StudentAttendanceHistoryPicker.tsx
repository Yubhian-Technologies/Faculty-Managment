...
// Entry point into the per-student attendance history report - pick a
// student here, same roster the Students page lists, then open their
// Monthly/Period/Till now report at `${hrefBase}/${student.id}/attendance`.
export function StudentAttendanceHistoryPicker({
  hrefBase = "/hod/students",
  ...
  function openHistory(student: StudentRow) {
    router.push(`${hrefBase}/${student.id}/attendance?name=${encodeURIComponent(student.name)}`);
  }
