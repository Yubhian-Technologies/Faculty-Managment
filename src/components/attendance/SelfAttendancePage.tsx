...
// branching), extracted here so College Office/Exam Cell/College Staff don't
// need a third hand-copied version. The two pre-existing pages are left as
// they are rather than migrated onto this, to avoid touching a working flow
// outside this change's scope.
export function SelfAttendancePage({ title, description }: SelfAttendancePageProps) {
  const now = new Date();
          ...
          {summaryCards.map((c) => (
            <Card key={c.label}>
              <CardContent className="p-4">
                ...
                <p className="text-xs text-muted-foreground mt-0.5">of {summary.totalWorkingDays} working days</p>
              </CardContent>
            </Card>
          ))}
        </div>
                  ...
                  <div key={rec.id} className="flex items-center justify-between gap-3 px-4 py-3">
                    ...
                    <div className="text-right shrink-0">
                      {rec.checkIn || rec.checkOut ? (
                        <p className="text-xs text-muted-foreground">{rec.checkIn ?? "—"} – {rec.checkOut ?? "—"}</p>
                      ) : null}
                      {rec.remarks ? (
