...
export function QualificationFields({ value: rawValue, onChange, collegeType }: ModuleFieldsProps & { collegeType?: CollegeType }) {
  ...
  // the form shows that data and every onChange emits the new shape.
...
export function ExperienceFields({ value: rawValue, onChange, includeTeachingAssignment = true }: ModuleFieldsProps & { includeTeachingAssignment?: boolean }) {
  // Un-migrated Firestore docs still carry the legacy key names - lift them so
  // the form shows that data and every onChange emits the new shape.
  const value = normalizeAcademicProfile(rawValue);
  function set<K extends keyof FacultyProfileFields>(key: K, v: FacultyProfileFields[K]) {
...
export function MentorshipFields({
  value: rawValue, onChange, ownerFacultyId, ownerFacultyName,
      ...
      <RepeatingGroup
        ...
        renderRow={(item, update) => (
          <>
            ...
            {/* Only dateOfAward is written - the legacy scalar `year` on an old
                record is left as-is (read-only fallback) and never re-derived. */}
            <DateInput
              label="Date of Award"
...
export function FinancialFields({ value: rawValue, onChange }: ModuleFieldsProps) {
  const value = normalizeAcademicProfile(rawValue);
  function set<K extends keyof FacultyProfileFields>(key: K, v: FacultyProfileFields[K]) {
    onChange({ ...value, [key]: v });
  }
