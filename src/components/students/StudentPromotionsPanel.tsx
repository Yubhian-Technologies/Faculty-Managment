...
import { ArrowRight, Check, ChevronsUpDown, GraduationCap, Users, Upload, FileSpreadsheet, AlertTriangle } from "lucide-react";
...
// A section's searchable text - department, section name, numeric/ordinal
// year, and batch - so typing any of "ECE", "Section A", "1st year", "2026"
// or "2026-2030" narrows the list live, with no separate search button.
function sectionSearchText(s: Section): string {
  return [s.department, s.name, `section ${s.name}`, String(s.year), yearOrdinalLabel(s.year), s.batch]
    .filter(Boolean)
    .join(" ")
    .toLowerCase();
}

function matchesSectionQuery(s: Section, query: string): boolean {
  const terms = query.trim().toLowerCase().split(/\s+/).filter(Boolean);
  if (terms.length === 0) return true;
  const haystack = sectionSearchText(s);
  return terms.every((term) => haystack.includes(term));
}

// Searchable section picker (Popover + Command) used for the source section
// and every target-section choice below - a plain Select doesn't support
// typing to filter, and with dozens of sections across departments/years/
// batches, scrolling a flat dropdown doesn't scale.
function SectionCombobox({
  ...
  const filtered = useMemo(() => sections.filter((s) => matchesSectionQuery(s, search)), [sections, search]);
    ...
    <Popover open={open} onOpenChange={(o) => { setOpen(o); if (!o) setSearch(""); }}>
      ...
      <PopoverContent className="w-[--radix-popover-trigger-width] p-0" align="start">
        <Command shouldFilter={false}>
          <CommandInput placeholder="Search by section, department, year or batch..." value={search} onValueChange={setSearch} />
          <CommandList>
            <CommandEmpty>{emptyText}</CommandEmpty>
            <CommandGroup>
              {filtered.map((s) => (
                <CommandItem key={s.id} value={s.id} onSelect={() => { onValueChange(s.id); setOpen(false); setSearch(""); }}>
...
// Shared between the standalone /principal/promotions route (own PageHeader)
// and the "Promotion" sub-tab of the Students page (showHeader=false there,
// since the parent Students page already renders one header for all tabs).
export function StudentPromotionsPanel({ showHeader = true }: { showHeader?: boolean } = {}) {
  ...
  const sortedSections = useMemo(
    () => sections.slice().sort((a, b) => a.year - b.year || a.department.localeCompare(b.department) || a.name.localeCompare(b.name)),
    [sections]
  );
      ...
      <Card>
        <CardContent className="p-5 space-y-2">
          ...
          <SectionCombobox
            ...
            placeholder="Choose the section/cohort to promote"
            className="max-w-md"
            formatLabel={(s) => `${s.department || "(no department)"} - Section ${s.name} · ${yearOrdinalLabel(s.year)} (${s.batch})`}
          />
        </CardContent>
      </Card>
            ...
            <CardContent className="p-5 space-y-3">
              ...
              {!isFinalYear && (
                <SectionCombobox
                  ...
                  value={defaultTarget}
                  onValueChange={applyDefaultToAll}
                  disabled={targetSections.length === 0}
                  placeholder="Select a default target section"
                  emptyText={`No sections exist yet for ${nextYear != null ? yearOrdinalLabel(nextYear) : "next year"}`}
                  className="max-w-md"
                  formatLabel={(s) => `${s.department || "(no department)"} - Section ${s.name}`}
                />
              )}
            </CardContent>
          </Card>
                          ...
                          <td className="px-3 py-2">
                            {isFinalYear ? (
                              ...
                              <SectionCombobox
                                ...
                                formatLabel={(sec) => targetLabel(sec.id)}
                              />
                            )}
                          </td>
                        </tr>
