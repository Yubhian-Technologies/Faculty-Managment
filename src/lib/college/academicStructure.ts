...
/**
 * The admission batch a given year of study belongs to, as its starting year.
 * A 2nd-year student in session 2026-27 was admitted in 2025, so
 * batchStartYear(2026, 2) === 2025. This is what makes batch-based
 * regulations self-maintaining: the same stored mapping resolves to the next
 * cohort automatically once the session rolls over.
 */
export function batchStartYear(sessionStartYear: number, studyYear: number): number {
  return sessionStartYear - (studyYear - 1);
}

/** "2026-2030" for a 4-year course admitted in 2026. */
export function batchLabel(startYear: number, durationYears: number): string {
  return `${startYear}-${startYear + durationYears}`;
}

/**
 * The regulation a year of study follows, resolved batch-first:
 *
 *  1. the batch that year of study belongs to (batchRegulations), which is
 *     the intended source of truth once a college has configured batches;
 *  2. failing that, whatever regulationsForYear says - the older year-range
 *     mapping - so nothing changes for a college that hasn't.
 *
 * Returns every candidate rather than one, because the fallback genuinely can
 * be ambiguous (two regulations both unrestricted for a year); a batch match
 * is always exactly one. Callers auto-select when there's a single candidate.
 */
export function regulationsForStudyYear(
  catalogItem: Pick<CourseCatalogItem, "regulations" | "regulationYears" | "batchRegulations" | "durationYears"> | null | undefined,
  studyYear: number,
  sessionStartYear: number
): { regulations: string[]; batchStartYear?: number; source: "batch" | "year-range" } {
  const start = batchStartYear(sessionStartYear, studyYear);
  const mapped = catalogItem?.batchRegulations?.[String(start)];
  // Only honoured while the code is still one this course actually offers -
  // a regulation removed from the course shouldn't linger via an old batch.
  if (mapped && (catalogItem?.regulations ?? []).includes(mapped)) {
    return { regulations: [mapped], batchStartYear: start, source: "batch" };
  }
  return { regulations: regulationsForYear(catalogItem, studyYear), batchStartYear: start, source: "year-range" };
}

/**
 * Validates a CourseCatalogItem's `batchRegulations` map on write: every value
 * must be one of the course's own `regulations`, and every key a plausible
 * 4-digit admission year. Mirrors validateRegulationYears below.
 */
export function validateBatchRegulations(
  batchRegulations: Record<string, string> | undefined,
  regulations: string[]
): string | null {
  if (!batchRegulations) return null;
  for (const [start, code] of Object.entries(batchRegulations)) {
    const year = Number(start);
    if (!Number.isInteger(year) || year < 1900 || year > 2200) {
      return `"${start}" is not a valid batch starting year`;
    }
    if (!regulations.includes(code)) {
      return `Batch ${start} is assigned regulation "${code}", which this course doesn't offer`;
    }
  }
  return null;
}
