...
import type { OfferLetterData, AppointmentLetterData } from "@/lib/pdf/offerLetterTemplate";
...
export async function downloadAppointmentLetterPdf(
  ...
  const res = await fetch("/api/pdf/generate", {
    method: "POST",
    headers: { "Content-Type": "application/json", Authorization: `Bearer ${token}` },
    body: JSON.stringify({ type: "APPOINTMENT_LETTER", data }),
  });
  if (!res.ok) throw new Error("PDF generation failed");
  ...
  link.download = `appointment-letter-${safeHint}.${blob.type.includes("pdf") ? "pdf" : "html"}`;
  link.click();
  URL.revokeObjectURL(url);
}
