...
// A faculty member's own "what am I responsible for" landing - populated
// only when an HOD has delegated a course-year's Timetable and Teaching
// Assignments to them (see TimetableIncharge in src/types/core.ts and the
// HOD's own assignment UI on hod/timetable/[courseId]/[year]/page.tsx). The
// HOD keeps full access too - this is a co-editor grant, not a handoff.
              ...
              <CardContent className="p-4 space-y-3">
                ...
                <div className="flex flex-col gap-2">
                  <Button variant="outline" size="sm" asChild className="justify-between">
                    <Link href={`/panel/timetable-incharge/${i.courseId}/${i.year}/teaching-assignments`}>
                      <span className="flex items-center gap-1.5"><ClipboardList className="h-3.5 w-3.5" />Teaching Assignments</span>
                      <ChevronRight className="h-3.5 w-3.5" />
                    </Link>
                  </Button>
                  <Button variant="outline" size="sm" asChild className="justify-between">
                    <Link href={`/panel/timetable-incharge/${i.courseId}/${i.year}`}>
