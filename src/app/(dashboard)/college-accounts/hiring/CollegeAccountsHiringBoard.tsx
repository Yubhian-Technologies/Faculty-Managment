...
// Identical to AccountsPipelineBoard's PipelineCard (5-step stepper +
// expandable candidate/session detail) - this board only adds department
// grouping around the same cards, see CollegeAccountsHiringBoard below.
...
// Groups entries by department name (the same string join key every hiring
// doc denormalizes - see VacancyRequest.department), sorted alphabetically.
// Departments with no requests in the current scope simply don't appear.
function groupByDepartment(entries: PipelineEntry[]): { department: string; entries: PipelineEntry[] }[] {
  const map = new Map<string, PipelineEntry[]>();
  for (const e of entries) {
    const dept = e.vacancy.department || "Unassigned";
...
export function CollegeAccountsHiringBoard({ scope }: { scope: "active" | "closed" }) {
  ...
  const grouped = groupByDepartment(visible);
  ...
  return (
    <div className="space-y-8">
      {grouped.map((g) => (
        <div key={g.department} className="space-y-3">
          <div className="flex items-center gap-2">
            <h2 className="text-sm font-semibold uppercase tracking-wider text-muted-foreground">{g.department}</h2>
            <span className="text-xs text-muted-foreground bg-muted px-2 py-0.5 rounded-full">{g.entries.length}</span>
          </div>
          {g.entries.map((e) => (
            <PipelineCard
              key={e.vacancy.id}
