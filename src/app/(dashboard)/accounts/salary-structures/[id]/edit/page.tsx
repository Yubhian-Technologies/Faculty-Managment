...
export default function EditSalaryStructurePage() {
  ...
  useEffect(() => {
    fetch("/api/college/salary-structures")
      ...
      .then((d) => {
        const s = (d.salaryStructures ?? []).find((item) => item.id === structureId);
        if (!s) {
          toast({ variant: "destructive", title: "Salary structure not found" });
      ...
      .catch(() => {
        toast({ variant: "destructive", title: "Failed to load salary structure" });
        router.push("/accounts/salary-structures");
      })
      .finally(() => setLoading(false));
  }, [structureId, router, reset]);
  ...
  const onSubmit = async (data: SalaryStructureFormData) => {
    ...
    try {
      const res = await fetch("/api/college/salary-structures", {
        ...
        body: JSON.stringify({ id: structureId, ...data }),
      });
      if (!res.ok) {
  ...
  if (loading) {
    return (
      <div className="max-w-xl">
        <PageHeader title="Edit Salary Structure" description="Loading…" />
      </div>
    );
  }
  ...
  return (
    <div className="max-w-xl">
      <PageHeader
        ...
        description="Update the pay template for this designation & employment type"
      />
          ...
          <form onSubmit={handleSubmit(onSubmit)} className="space-y-4">
            ...
            <div className="grid grid-cols-2 gap-3">
              <div className="space-y-2">
                ...
                <Select defaultValue={watched.designation} onValueChange={(v) => setValue("designation", v)}>
                  <SelectTrigger>
              ...
              <div className="space-y-2">
                ...
                <Select defaultValue={watched.employmentType} onValueChange={(v) => setValue("employmentType", v)}>
                  <SelectTrigger>
