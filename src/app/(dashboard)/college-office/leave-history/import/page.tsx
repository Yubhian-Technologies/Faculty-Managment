...
export default function LeaveHistoryImportPage() {
  ...
  async function handleFile(e: React.ChangeEvent<HTMLInputElement>) {
    ...
    try {
      ...
      if (!Object.values(keyMap).includes("employeeId") && !Object.values(keyMap).includes("employeeName")) {
        setParseError("Couldn't find an \"Employee Code\" or \"Employee Name\" column. Check your file's header row against the template.");
        return;
      }
  ...
  const missingRequired = rows.length > 0
    ? rows.some((r) => requiredKeys.some((k) => !r[k]?.trim()) || (!r.employeeId?.trim() && !r.employeeName?.trim()))
    : false;
  ...
  return (
    <div className="max-w-5xl space-y-6">
      <PageHeader
        ...
        description="Bulk-record leave already taken - before this module existed, or any other gap in the record"
        actions={
      ...
      <Card>
        ...
        <CardContent className="space-y-4">
          <p className="text-sm text-muted-foreground">
            Download the CSV template, fill in each employee&apos;s past leave records, and upload it below.
          </p>
              ...
              <table className="w-full text-xs">
                ...
                <tbody>
                  {rows.slice(0, 20).map((row, i) => {
                    const missing = requiredKeys.some((k) => !row[k]?.trim()) || (!row.employeeId?.trim() && !row.employeeName?.trim());
                    return (
          ...
          <CardContent className="p-5 space-y-4">
            ...
            {result.created > 0 && (
              <Button asChild variant="outline" size="sm">
                <Link href="/college-office/leave-history">View Leave History</Link>
              </Button>
            )}
          </CardContent>
        </Card>
