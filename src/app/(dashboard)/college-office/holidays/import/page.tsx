...
import { HOLIDAY_IMPORT_COLUMNS as COLUMNS, HOLIDAY_IMPORT_HINTS as HINTS } from "@/lib/college/holidayImportColumns";
...
export default function HolidaysImportPage() {
  ...
  function downloadTemplate() {
    ...
    downloadCSV(toCSV([headers, sample1]), "holidays_import_template.csv");
  }
  ...
  async function handleFile(e: React.ChangeEvent<HTMLInputElement>) {
    ...
    try {
      ...
      if (!Object.values(keyMap).includes("occasion") || !Object.values(keyMap).includes("date")) {
        setParseError("Couldn't find both an \"Occasion\" and a \"Date\" column. Check your file's header row against the template.");
        return;
      }

      setIgnoredHeaders(getUnmatchedHeaders(headers, keyMap));
  ...
  async function handleImport() {
    ...
    setResult(null);
    try {
      const res = await fetch("/api/college/holidays/import", {
        method: "POST",
      ...
      setResult(json);
      if (json.created > 0) {
        toast({ variant: "success", title: `${json.created} holiday${json.created !== 1 ? "s" : ""} imported successfully` });
        setRows([]);
      }
    } catch {
  ...
  return (
    <div className="max-w-5xl space-y-6">
      <PageHeader
        ...
        description="Bulk-add the academic calendar's holidays at once instead of one at a time"
        actions={
          <Button variant="outline" asChild>
            <Link href="/college-office/holidays"><ArrowLeft className="h-4 w-4 mr-1" />Back to Holidays</Link>
          </Button>
        }
      />
      ...
      <Card>
        ...
        <CardContent className="space-y-4">
          <p className="text-sm text-muted-foreground">
            Download the CSV template, fill in the academic calendar&apos;s holidays, and upload it below.
          </p>
        ...
        <Card className={result.created > 0 ? "border-green-200" : "border-red-200"}>
          <CardContent className="p-5 space-y-4">
            <div className="flex items-center gap-3">
              ...
              <div>
                <p className="font-semibold">{result.created} holiday{result.created !== 1 ? "s" : ""} imported successfully</p>
                ...
                <div className="rounded-lg border divide-y max-h-48 overflow-y-auto">
                  {result.failed.map((f, i) => (
                    <div key={i} className="flex items-center justify-between px-3 py-2 text-sm gap-3">
                      <span className="text-muted-foreground shrink-0">Row {f.row} · {f.identifier}</span>
            ...
            {result.created > 0 && (
              <Button asChild variant="outline" size="sm">
                <Link href="/college-office/holidays">View Holidays</Link>
              </Button>
            )}
          </CardContent>
        </Card>
