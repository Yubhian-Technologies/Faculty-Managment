...
export default function PrincipalStaffModuleEditPage() {
  ...
  useEffect(() => {
    fetch(`/api/college/users/${uid}`)
      ...
      .then((data) => {
        if (!data.user) {
          toast({ variant: "destructive", title: data.error ?? "Staff account not found" });
        ...
        <PageHeader
          ...
          actions={
            <Button variant="outline" asChild>
              <Link href={`/principal/staff/${uid}/${moduleKey}`}><ArrowLeft className="h-4 w-4 mr-2" />Back</Link>
            </Button>
          }
        />
