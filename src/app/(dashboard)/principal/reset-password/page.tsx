...
export default function CollegeAdminResetPasswordPage() {
  ...
  async function load() {
    setIsLoading(true);
    try {
      const rows = await fetch("/api/college/users?includeAll=true").then((r) => r.json() as Promise<{ users: UserRow[] }>).then((d) => d.users ?? []);
      setUsers(rows);
    } catch {
  ...
  useEffect(() => { if (isCollegeAdmin) void load(); }, [isCollegeAdmin]);

  function openResetDialog(target: UserRow) {
    setPasswordInput("");
  ...
  async function handleReset() {
    ...
    setIsResetting(true);
    try {
      const res = await fetch("/api/college/college-admin/reset-password", {
        method: "POST",
  ...
  if (!isCollegeAdmin) {
    return (
      <div className="space-y-6">
        <PageHeader title="Reset Member Password" description="Not available" />
        <p className="text-sm text-muted-foreground">This page is only available to a College Admin login.</p>
      </div>
    );
  }
  ...
  return (
    <div className="space-y-6">
      <PageHeader title="Reset Member Password" description="Every login in this college - reset a password if someone is locked out" />
