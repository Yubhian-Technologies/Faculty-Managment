...
// "CSE-CSE-A". Mirrors attendance-reports/[departmentId]/[courseId]/
// page.tsx's own formatSectionLabel.
...
// Passes the section's own raw `name`/`department`/`year` through (not just
// the formatted display label) - the next step needs them to filter
// GET /api/college/students, which keys off the raw section name.
export default function PrincipalAttendanceHistorySectionsPage() {
  ...
  return (
    <div className="space-y-6">
      <PageHeader
        title={courseLabel}
        description="Pick a section to view its students."
        actions={
          <Button
            variant="outline"
            onClick={() =>
              router.push(`/principal/attendance-history/${departmentId}?deptLabel=${encodeURIComponent(deptLabel)}`)
            }
          >
          ...
          {sections.map((s) => {
            ...
            return (
              <Card
                ...
                onClick={() =>
                  router.push(
                    `/principal/attendance-history/${departmentId}/${courseId}/${s.id}`
                    + `?deptLabel=${encodeURIComponent(deptLabel)}&courseLabel=${encodeURIComponent(courseLabel)}`
                    + `&sectionLabel=${encodeURIComponent(label)}&sectionName=${encodeURIComponent(s.name)}&sectionYear=${s.year}`
                  )
                }
              >
