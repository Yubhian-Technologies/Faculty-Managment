...
export default function HODLeavePage() {
  ...
  function getBalance(code: LeaveTypeCodeV2) {
    ...
    if (!b) return null;
    return {
      available: Math.max(0, b.opening + b.credited - b.used - b.pending),
      used: b.used,
      pending: b.pending,
    };
  }
  ...
  return (
    <div className="space-y-6">
      <PageHeader
        ...
        actions={
          <Button onClick={() => router.push("/hod/leave/apply")} disabled={!profile || loading}>
      ...
      {!loading && !profile && (
        <div className="flex items-start justify-between gap-3 rounded-lg border border-amber-200 bg-amber-50 p-4">
          ...
          <Button size="sm" variant="outline" onClick={() => router.push("/hod/leave/setup")} className="shrink-0 border-amber-400 text-amber-800 hover:bg-amber-100">
            Setup Profile
          </Button>
        </div>
      )}
