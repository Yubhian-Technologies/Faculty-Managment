...
import { AlertCircle, ChevronRight, Info, Paperclip, X } from "lucide-react";
import type {
  LeaveTypeFull, LeaveTypeCodeV2, LeaveBalanceV2,
  EmployeeLeaveProfile, ValidationResult,
} from "@/types/leave";
...
export default function HODLeaveApplyPage() {
  ...
  useEffect(() => {
    if (!selectedType || !fromDate || !toDate || !profile) { setErrors([]); setWarnings([]); return; }
  ...
  const canSubmit =
    ...
    !!reason.trim() && !!leaveAddress.trim() && !!contactNumber.trim() &&
    errors.length === 0 && !submitting && !certUploading;
    ...
    try {
      const res = await fetch("/api/leave/applications", {
        ...
        body: JSON.stringify({
          ...
          reason, leaveAddress, contactNumber,
      ...
      toast({ variant: "success", title: "Leave application submitted", description: `${computedDays} day(s) sent for review.` });
  ...
  return (
    <div className="max-w-2xl">
      <PageHeader title="Apply for Leave" description="Fill in the details to submit a leave application" />

      {!profile && !loadingMeta && (
        <div className="mb-4 flex items-start gap-3 rounded-lg border border-amber-200 bg-amber-50 p-4">
          <AlertCircle className="h-5 w-5 text-amber-600 shrink-0 mt-0.5" />
          <p className="text-sm text-amber-800">Your leave profile is not set up. Contact HR to initialize it.</p>
        </div>
      )}

      <form onSubmit={(e) => void handleSubmit(e)} className="space-y-5">
        <Card>
          <CardHeader><CardTitle className="text-base">Leave Type</CardTitle></CardHeader>
              ...
              {leaveTypes.map((lt) => {
                ...
                return (
                  <button
                    key={lt.code}
                    type="button"
                    onClick={() => { setSelectedCode(lt.code); setIsHalfDay(false); }}
        ...
        {selectedType && (
          <Card>
            <CardHeader><CardTitle className="text-base">Leave Period</CardTitle></CardHeader>
            <CardContent className="space-y-4">
              <div className="flex items-center gap-3">
                ...
                {isHalfDay && (
                  <Select value={halfDaySession} onValueChange={(v) => setHalfDaySession(v as "MORNING" | "AFTERNOON")}>
              ...
              <div className={`grid gap-4 ${isHalfDay ? "grid-cols-1" : "grid-cols-2"}`}>
                <div className="space-y-2">
                  <Label>{isHalfDay ? "Date" : "From Date"} *</Label>
                  <Input
        ...
        {selectedType && fromDate && (isHalfDay || toDate) && (
          <Card>
            ...
            <CardContent className="space-y-4">
              <div className="space-y-2">
                <Label>Reason *</Label>
                <Textarea value={reason} onChange={(e) => setReason(e.target.value)} placeholder="Brief reason..." rows={3} />
              </div>
              <div className="space-y-2">
                <Label>Address during leave *</Label>
                <Input value={leaveAddress} onChange={(e) => setLeaveAddress(e.target.value)} placeholder="Where can you be reached?" />
              </div>
              <div className="space-y-2">
                <Label>Contact number during leave *</Label>
                <Input value={contactNumber} onChange={(e) => setContactNumber(e.target.value)} placeholder="Mobile / landline" />
              </div>
              {selectedType.rules.requiresHandoverAfterDays !== undefined && computedDays >= selectedType.rules.requiresHandoverAfterDays && (
                <div className="space-y-2">
                  <Label>Substitute / handover arrangement</Label>
                  <Textarea value={substituteArrangement} onChange={(e) => setSubstituteArrangement(e.target.value)} placeholder="Who will cover your duties?" rows={2} />
                </div>
              )}
              <label className="flex items-start gap-2 cursor-pointer">
                <input type="checkbox" checked={otherEmploymentAck} onChange={(e) => setOtherEmploymentAck(e.target.checked)} className="mt-1 h-4 w-4 rounded" />
                ...
                <div className="space-y-2">
                  ...
                  {certUrl ? (
                    <div className="flex items-center gap-2 rounded-md border border-green-200 bg-green-50 px-3 py-2">
                      ...
                      <button type="button" onClick={() => { setCertFile(null); setCertUrl(null); }} className="text-green-600 hover:text-green-800">
                        <X className="h-4 w-4" />
                      </button>
                    </div>
                  ) : (
                    <label className="block cursor-pointer rounded-md border border-dashed border-muted-foreground/40 px-3 py-2 text-center hover:border-muted-foreground/60 transition-colors">
                      <input type="file" accept="application/pdf" className="sr-only" disabled={certUploading}
                        onChange={(e) => { const f = e.target.files?.[0]; if (f) void handleCertUpload(f); e.target.value = ""; }} />
        ...
        {selectedType && (
          <div className="flex flex-col-reverse gap-3 sm:flex-row sm:justify-end">
            ...
            <Button type="submit" disabled={!canSubmit} loading={submitting}>Submit Application</Button>
          </div>
        )}
      </form>
    </div>
