...
// different range passed to /api/college/student-attendance-history.
export default function StudentAttendanceHistoryPage() {
  const router = useRouter();
  const { studentId } = useParams<{ studentId: string }>();
  const searchParams = useSearchParams();
  const nameParam = searchParams.get("name") || "Student";
  ...
  // the matching per-student breakdown immediately instead of landing on
  // the picker and making the HOD re-pick the same month/range and hit Show
  // again. Absent entirely when reached from the plain Attendance History
  // list (no student picked a range yet).
  ...
  // picker again would just be noise before the sheet the HOD actually
  // asked for. Starts collapsed in that case; a "Change range" link brings
  // it back if they want a different range without navigating away.
  ...
  useEffect(() => {
    ...
    // state change would fight the HOD's own later picker edits.
    void (async () => {
      await handleShow();
    })();
        ...
        subjects.length === 0 ? (
          <Card>
            <CardContent className="py-12 text-center text-sm text-muted-foreground">
              No attendance records for {nameParam} in this range.
            </CardContent>
          </Card>
        ) : (
