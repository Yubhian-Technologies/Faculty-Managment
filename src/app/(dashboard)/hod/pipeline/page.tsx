...
function getNextAction(entry: PipelineEntry): NextAction {
  ...
  if (p === "COLLEGE_OFFICE_SETUP") {
    return { label: "Waiting for Office Setup", href: "#", disabled: true };
  }
...
export default function HiringPipelinePage() {
  ...
  useEffect(() => {
    void Promise.all([
      ...
      .then(([vacancies, candidates, batches]) => {
        ...
        // Active first (not rejected, not completed), then closed
        built.sort((a, b) => {
          const aActive =
            a.vacancy.status !== "REJECTED" && a.batch?.currentPhase !== "COMPLETED" ? 0 : 1;
          const bActive =
            b.vacancy.status !== "REJECTED" && b.batch?.currentPhase !== "COMPLETED" ? 0 : 1;
  ...
  const active = entries.filter(
    (e) => e.vacancy.status !== "REJECTED" && e.batch?.currentPhase !== "COMPLETED"
  );
  ...
  return (
    <div className="space-y-6">
      <PageHeader
        ...
        description="All positions — from request to hiring decision in one view"
        actions={
          <Button asChild>
      ...
      {isLoading ? (
        ...
        <div className="rounded-xl border border-dashed p-14 text-center">
          ...
          <p className="font-semibold text-muted-foreground">No hiring requests yet</p>
          ...
          {active.length > 0 && (
            <section className="space-y-3">
              <div className="flex items-center gap-2">
                ...
                <span className="text-xs bg-primary text-primary-foreground font-bold px-1.5 py-0.5 rounded-full">
                  {active.length}
                </span>
              </div>
              {active.map((e) => (
