...
import { indentItemsTotal, INDENT_REQUEST_TYPE_LABELS, type IndentRequest, type IndentRequestType } from "@/types";

type OverviewIndent = IndentRequest & { collegeName: string; locationId: string; locationName: string };

export default function PurchaseByTypePage() {
  const searchParams = useSearchParams();
  const typeFilter = searchParams.get("requestType") as IndentRequestType | null;
  ...
  const filteredRequests = typeFilter ? requests.filter((r) => r.requestType === typeFilter) : requests;

  const grouped: Record<string, OverviewIndent[]> = {};
  for (const r of filteredRequests) {
    (grouped[r.requestType] ??= []).push(r);
  }
  ...
  return (
    <div className="space-y-6">
      <PageHeader
        title="Indents by Goods / Non-Goods"
        description={
      ...
      {(typeFilter || collegeId || locationId) && (
        <div className="flex flex-wrap items-center gap-2 text-sm">
          ...
          {typeFilter && (
            <>
              ...
              <Badge variant="outline">{INDENT_REQUEST_TYPE_LABELS[typeFilter] ?? typeFilter}</Badge>
            </>
          )}
          <Button variant="ghost" size="sm" asChild className="h-7 px-2">
            <Link href="/purchase/by-type"><X className="h-3 w-3 mr-1" />Clear</Link>
          </Button>
        </div>
      )}
      ...
      {!isLoading && filteredRequests.length === 0 && (
        <EmptyState
          ...
          description="Indents raised by HODs will appear here, grouped by Goods / Non-Goods."
            ...
            return (
              <div key={type} className="space-y-3">
                <div className="flex items-center gap-2">
                  ...
                  <h2 className="text-base font-semibold">{INDENT_REQUEST_TYPE_LABELS[type as IndentRequestType] ?? type}</h2>
