...
export async function POST(request: Request) {
  try {
    ...
    if (session.role !== "HOD") {
      ...
      // Principal, Vice Principal, College Admin, and Director (equal authority -
      // College Admin/Director mirror Principal's authority end-to-end, see
      // UserRole's own doc-comment; their sessions normalize to "PRINCIPAL" but
      // their own target docs keep the real "COLLEGE_ADMIN"/"DIRECTOR" role)
      // reset each other symmetrically, plus any of them can reset an HOD or a
      // unit head - mirrors manual/route.ts's identical PRINCIPAL/VICE_PRINCIPAL
      // treatment there.
      const validTargetRoles = ["PRINCIPAL", "VICE_PRINCIPAL", "COLLEGE_ADMIN", "DIRECTOR", "HOD", ...COLLEGE_STAFF_UNIT_HEAD_ROLES];
