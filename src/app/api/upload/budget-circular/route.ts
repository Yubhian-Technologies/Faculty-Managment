...
export async function POST(request: Request) {
  try {
    ...
    if (!ALLOWED_TYPES.includes(file.type)) {
      return NextResponse.json({ error: "Only PDF, Word, Excel, JPEG, or PNG files are accepted" }, { status: 400 });
    }
    ...
    const path = `colleges/${session.collegeId}/budgetCirculars/${filename}`;
    ...
    console.error("[upload/budget-circular POST]", err);
