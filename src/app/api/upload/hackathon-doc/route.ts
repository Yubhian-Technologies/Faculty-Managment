...
// Sanctioned Letter / Brochure / Expenditure Proof / Event Report - same
// pattern as /api/upload/consultancy-doc.
const KINDS = ["sanctioned-letter", "brochure", "expenditure-proof", "event-report"] as const;
...
export async function POST(request: Request) {
  try {
    ...
    const path = `hackathon-docs/${session.collegeId}/${session.uid}/${kind}/${randomUUID()}.${ext}`;
    ...
    console.error("[upload/hackathon-doc POST]", err);
