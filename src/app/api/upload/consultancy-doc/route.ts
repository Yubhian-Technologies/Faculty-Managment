...
// A consultancy project's three supporting documents (Proof of Faculty
// Share, Completion Report, Supporting Documents of Income) - uploaded while
// filling out the self-submit form, before the project record itself exists,
// so (like /api/upload/certificate) this can't tie the storage path to an
// existing document id. `kind` just picks a readable subfolder; every path
// still starts with the uploader's own uid.
const KINDS = ["faculty-share-proof", "completion-report", "income-proof"] as const;
...
export async function POST(request: Request) {
  try {
    ...
    const path = `consultancy-docs/${session.collegeId}/${session.uid}/${kind}/${randomUUID()}.${ext}`;
    ...
    console.error("[upload/consultancy-doc POST]", err);
