...
// Sanctioned Letter / Brochure / Schedule / Completion Report / Conference
// Proceedings / Reviewer Certificate / Editor Appointment Letter - same
// pattern as /api/upload/consultancy-doc.
const KINDS = [
  "sanctioned-letter", "brochure", "schedule", "completion-report", "conference-proceedings",
  "reviewer-certificate", "appointment-letter",
] as const;
...
export async function POST(request: Request) {
  try {
    ...
    const path = `research-service-docs/${session.collegeId}/${session.uid}/${kind}/${randomUUID()}.${ext}`;
    ...
    console.error("[upload/research-service-doc POST]", err);
