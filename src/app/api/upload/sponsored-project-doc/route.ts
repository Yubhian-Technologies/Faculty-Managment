...
// Progress Report / Completion Report / Utilization Certificate / Statement
// of Expenditure - uploaded while filling out the self-submit form, before
// the sponsored-project record exists, same pattern as
// /api/upload/seed-funding-doc.
const KINDS = ["progress-report", "completion-report", "utilization-certificate", "statement-of-expenditure"] as const;
...
export async function POST(request: Request) {
  try {
    ...
    const path = `sponsored-project-docs/${session.collegeId}/${session.uid}/${kind}/${randomUUID()}.${ext}`;
    ...
    console.error("[upload/sponsored-project-doc POST]", err);
