...
// Allotment Order / Awarded Degree proof - same pattern as
// /api/upload/consultancy-doc.
const KINDS = ["allotment-order", "awarded-degree"] as const;
...
export async function POST(request: Request) {
  try {
    ...
    const path = `phd-docs/${session.collegeId}/${session.uid}/${kind}/${randomUUID()}.${ext}`;
    ...
    console.error("[upload/phd-doc POST]", err);
