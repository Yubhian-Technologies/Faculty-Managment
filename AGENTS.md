...
- `src/app/api/` is split by scope, mirroring tenancy: `admin/` (super admin - colleges, locations, users, settings, audit-logs, general-admin-vacancies), `college/` (college-scoped - faculty, departments, sections, candidates, hiring-batches, vacancy-requests, offer-letters, attendance, teaching-assignments, salary-records, notifications, budget-requests, finance-* endpoints), `location/` (location-scoped - users, departments, candidates, vacancy-requests, interviews, offers), `administration/principals`, plus `auth/`, `email/`, `pdf/`, `upload/`, `public/`.
